...
(function (global, factory) {
  ...
  function setupDOM(handler) {
    if (!handler.ptrElement) {
      ...
      // }
      var mm = document.getElementById('msg').append(ptr)
  ...
  var _ptrMarkup = "\n<div class=\"__PREFIX__box\">\n  <div class=\"__PREFIX__content\">\n    <div class=\"__PREFIX__icon\" style='color:white;'></div>\n    <div class=\"__PREFIX__text\" style='color:white;'></div>\n  </div>\n</div>\n";
  ...
  var _defaults = {
    ...
    onRefresh: function () {
      if($('.box').first()[0]!=null){  
      pst = $('.box').first().attr('id','here')
      var stay_the_scroll=true
      }
      $.ajax({
        type:'GET',
        url:'http://10.20.30.7:8888/api/requestchathistory',
        data:{
            join_time:join_time,
        },
        dataType:'json',
        success:function(data){
          var msgs_arry = data.result
          if(msgs_arry.length==0){
            pop_window('已经是顶部啦！')
          }else
          {$(msgs_arry).each(function(index){
                        
            var tiMe 
            last_time = parseInt(msgs_arry[index].Date,10)
            last_time *=1000
            if (this_time-last_time >= 360000){
              tiMe = last_time
            ...
            var info=[msgs_arry[index].content,msgs_arry[index].user_id,msgs_arry[index].name,tiMe,msgs_arry[index].icon_id]  
            append_chat_history(info)
            join_time = msgs_arry[index].Date
          })}
          if(stay_the_scroll==true){
            console.log('scroolppp')
          $('#msg').scrollTop($('#here')[0].offsetTop)
          }
